...
void i2c_write(uint8_t addr, uint8_t reg_addr, uint8_t *data, size_t len) {
  ESP_LOGE("I2C", "Write slave 0x%X reg 0x%X size %zu data[0] 0x%X",
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGE("I2C", "I2C write success");
  }
...
void i2c_read(uint8_t addr, uint8_t reg_addr, uint8_t *dest, size_t len) {
  ESP_LOGE("I2C", "Read slave 0x%X reg 0x%X size %zu", (addr << 1), reg_addr,
           len);
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGE("I2C", "I2C read success");
  }

  ESP_LOGE("I2C", "Read slave 0x%X reg 0x%X data %s", addr, reg_addr, dest);
